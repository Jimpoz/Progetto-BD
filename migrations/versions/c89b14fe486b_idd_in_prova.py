...
Revision ID: c89b14fe486b
Revises: 649ae161f209
Create Date: 2023-05-23 13:38:35.563495
...
revision = 'c89b14fe486b'
down_revision = '649ae161f209'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    with op.batch_alter_table('prova', schema=None) as batch_op:
        batch_op.add_column(sa.Column('idD', sa.String(length=100), nullable=True))
        batch_op.create_foreign_key('fk_prova_idD_docente', 'docente', ['idD'], ['idD'])
...
def downgrade():
    ...
    op.create_table('_alembic_tmp_prova',
    sa.Column('idP', sa.INTEGER(), nullable=False),
    sa.Column('tipo_voto', sa.VARCHAR(length=100), nullable=True),
    sa.Column('idE', sa.VARCHAR(length=100), nullable=True),
    sa.Column('tipo_prova', sa.VARCHAR(length=8), nullable=True),
    sa.ForeignKeyConstraint(['idE'], ['esame.idE'], ),
    sa.PrimaryKeyConstraint('idP')
    )
